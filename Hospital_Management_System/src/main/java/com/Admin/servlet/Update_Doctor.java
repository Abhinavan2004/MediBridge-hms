...
public class Update_Doctor extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (dao.update_doctor(doc_name, doc_dob, doc_quali, doc_spec, doc_email, doc_contact , doc_password , id)) {
				session.setAttribute("succMsg", "Doctor Updated Successfully");
				System.out.println("Doctor Updated Successfully");
				resp.sendRedirect("admin/doctor_dashboard.jsp");
			} else {
				session.setAttribute("errorMsg", "Doctor Not Updated Successfully");
				System.out.println("Doctor Not Updated Successfully");
				resp.sendRedirect("admin/doctor_dashboard.jsp");
			}
		} catch (Exception e) {
			e.printStackTrace();
